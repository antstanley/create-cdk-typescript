...
const tsconfigNoExcludeJson = JSON.stringify({
...
describe('add correct scripts to tsconfig.json', () => {
	beforeAll(() => {
		...
		writeFileSync(
			join(testPaths['exclude'], 'tsconfig.json'),
			tsconfigExcludeJson
		)
		writeFileSync(
			join(testPaths['no-exclude'], 'tsconfig.json'),
			tsconfigNoExcludeJson
		)
		writeFileSync(
			join(testPaths['exclude-comments'], 'tsconfig.json'),
			tsconfigExcludeComments
		)
	})
	...
	test('update tsconfig.json where an exclude property exists', () => {
		...
		const getTsConfigJson = JSON.parse(
			readFileSync(join(testPaths['exclude'], 'tsconfig.json'), 'utf8')
		)
	...
	test('update tsconfig.json where no exclude property exists', () => {
		...
		expect(result).toBe(true)
		const getTsConfigJson = JSON.parse(
			readFileSync(join(testPaths['no-exclude'], 'tsconfig.json'), 'utf8')
		)
		expect(getTsConfigJson).toHaveProperty('exclude')
		expect(getTsConfigJson?.exclude).toContain(excludeGlob)
	})
	...
	test('update tsconfig.json where an exclude property exists and there are comments', () => {
		...
		expect(result).toBe(true)
		const getTsConfigJson = JSON.parse(
			readFileSync(join(testPaths['exclude-comments'], 'tsconfig.json'), 'utf8')
		)
		expect(getTsConfigJson).toHaveProperty('exclude')
		expect(getTsConfigJson?.exclude).toContain(excludeGlob)
	})
